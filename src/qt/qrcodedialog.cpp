...
{
    QString ret = QString("talant:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non TLT unit input in lnReqAmount, we generate the URI with TLT as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(TalantUnits::format(TalantUnits::TLT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
